...
class S3Utils:
    ...
    def read_data_from_bucket(self, bucket_name, partner, file_path) -> \
        ...
        logging.info(f'Fetching data for partner {partner} from file path {file_path}')
        ...
        try:
            for obj in bucket.objects.all():
                if file_path == obj.key:
                    ...
                    logging.info('Completed reading data from above mentioned path')
        except Exception as e:
            logging.critical(e)
            logging.critical(
...
class Detection:
    ...
    def detect(self, delta_dict, bulk):
        ...
        if bulk['snowflake_YoY_drop'].values[0] < -20 or bulk['snowflake_DoD_drop'].values[0] < -20: 
            logging.info(f' Anomaly Detected for partner {self.partner_name} ')
            anomaly_detected_partner_data = \
            ...
            lower_drop = ((delta_dict['snowflake_imp'] - bulk['lower_bound'].values[0])/delta_dict['snowflake_imp'])*100
            
            anomaly_detected_partner_data['lower_drop'] = round(lower_drop,2)
            date = bulk['utc_date'].values[0]
            logging.info(f' Anomaly Detected for partner {self.partner_name} for date {date}')
        else:
            date = bulk['utc_date'].values[0]
            logging.info(f' No Anomaly Detected for partner {self.partner_name} for date {date}')


        return anomaly_detected_partner_data

    def run(self):

        logging.info(f'Collecting delta data for partner: {self.partner_name }')
        path = 'partners_data_testing' + '/' + self.partner_name + '/' + self.partner_name + '.csv'
        ...
        if not partner_delta.empty:
            ...
            if partner_bulk is None:
                logging.info(f'Collecting bulk data for partner: {self.partner_name }')
                bulk_path = 'partners_data_testing' + '/' + self.partner_name + '/' + self.partner_name + '_bulk.csv'
...
# Convert named tuple to dictionary
                ...
                logging.info(f' Captured delta data for date : {partner_delta_date} ')

                logging.info(f'Collecting bulk data for partner: {self.partner_name}')
                bulk_path = 'partners_data_testing' + '/' + self.partner_name + '/' + self.partner_name + '_bulk.csv'

                partner_bulk_delta_date_record = partner_bulk.loc[partner_bulk['utc_date'] == partner_delta_date]
                partner_bulk_date = partner_bulk_delta_date_record['utc_date'].values[0]

                logging.info(f'Captured bulk data for date : {partner_bulk_date} ')
                ...
                if previous_year_at == 0:
                    ...
                    year_drop = ((row['athena_imp'] - previous_year_at)/previous_year_at)*100
                partner_bulk_delta_date_record['previous_yr_athena_imp'] = previous_year_at
                partner_bulk_delta_date_record['athena_YoY_drop'] = round(year_drop, 2)
                ...
                day_drop = ((row['snowflake_imp'] - previous_day_sf)/previous_day_sf)*100
                partner_bulk_delta_date_record['previous_day_snowflake_imp'] = previous_day_sf
                partner_bulk_delta_date_record['snowflake_DoD_drop'] = round(day_drop, 2)
        ...
        logging.info(f'Anomaly detection process completed for {self.partner_name} partner ')
        logging.info(f'Captured details for Anomaly partners are {anomaly_partner_data}')
        return anomaly_partner_data

partner = "spotify"
parent_bucket = "iasdl-datalake-raw-ue1-cre-dev"
detection = Detection(partner, parent_bucket)
anomaly_data = detection.run()
if anomaly_data:
    print(anomaly_data)
